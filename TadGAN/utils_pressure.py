...
def plot_ts(X, labels=None):
    ...
    plt.title('Pressure in Bamberg, weather station 282', size=16)
    plt.ylabel('Pressure', size=16)
    ...
    if labels:
        plt.legend(labels=labels, loc=1, prop={'size': 16})
...
def plot_rws(X, window=100, k=5, lim=1000): #was window=100
    shift = 75
    X = X[window:]
    t = range(len(X))
